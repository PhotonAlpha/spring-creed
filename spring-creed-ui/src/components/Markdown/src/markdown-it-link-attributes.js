...
function findFirstMatchingImgConfig(link, configs) {
  var i, config
  if (!link.attrIndex) {
    return
  }
  var href = link.attrs[link.attrIndex('src')][1]
...
function markdownitLinkAttributes(md, configs) {
  ...
  var defaultRenderImage = md.renderer.rules.image || this.defaultRender
  md.renderer.rules.image = function (tokens, idx, options, env, self) {
    var config = findFirstMatchingImgConfig(tokens[idx], configs)
    var attributes = config && config.attrs
    attributes.onclick = `window.open('${config.href}', '_blank')`
...
markdownitLinkAttributes.defaultRender = (tokens, idx, options, env, self) => {
