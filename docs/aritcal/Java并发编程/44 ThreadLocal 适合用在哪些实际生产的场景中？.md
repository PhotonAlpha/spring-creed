...
在通常的业务开发中，ThreadLocal 有**两种典型**的使用场景。
...
![img](./img/44/threadlocal_usage-1.png)
...
![img](./img/44/threadlocal_usage-2.png)
...
![img](./img/44/threadlocal_usage-3.png)
...
我们用下面的代码来演示**只用一个 simpleDateFormat** 对象的情况：
...
![img](./img/44/threadlocal_usage-4.png)
...
可以看出在 date 方法中加入了 `synchronized` 关键字，把 **simpleDateFormat 的调用**给上了锁。
...
![img](./img/44/threadlocal_usage-5.png)
...
我们用图画的形式举一个实例：
![img](./img/44/threadlocal_usage-6.png)
...
![img](./img/44/threadlocal_usage-7.png)
...
![img](./img/44/threadlocal_usage-8.png)
...
![img](./img/44/threadlocal_usage-9.png)
...
1. 场景1，ThreadLocal 用作**保存每个线程独享的对象**，为每个线程都创建一个副本，每个线程都只能修改自己所拥有的副本, 而不会影响其他线程的副本，这样就让原本在并发情况下，线程不安全的情况变成了线程安全的情况。

2. 场景2，ThreadLocal 用作**每个线程内需要独立保存信息**的场景，**供其他方法更方便得获取**该信息，每个线程获取到的信息都可能是不一样的，前面执行的方法设置了信息后，后续方法可以通过 ThreadLocal 直接获取到，避免了传参。
