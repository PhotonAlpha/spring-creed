...
- **（1）单线程规则**： 在一个单独的线程中，按照程序代码的顺序，先执行的操作 happen-before 后执行的操作。也就是说，如果操作 x 和操作 y 是同一个线程内的两个操作，并且在代码里 x 先于 y 出现，那么有 hb(x, y)，正如下图所示：

![happens-before](./img/61/happens-before-1.png)
...
![happens-before](./img/61/happens-before-2.png)
...
![happens-before](./img/61/happens-before-3.png)
...
![happens-before](./img/61/happens-before-4.png)
