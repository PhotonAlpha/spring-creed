...
### Runnable 可运行
![runnable](./img/03/03_Runnable_thread.png)
...
### 阻塞状态
![waitting](./img/03/03_%E9%98%BB%E5%A1%9E%E7%8A%B6%E6%80%81.png)
...
- #### Waiting 等待
![blocked](./img/03/03_waitting.png)
...
- #### Timed Waiting 限期等待
![blocked](./img/03/03_timed_waitting.png)
...
![blocked](./img/03/03_blocked_circulation.png)
想要从 Blocked 状态进入 Runnable 状态，要求线程获取 monitor 锁，而从 Waiting 状态流转到其他状态则比较特殊，因为首先 Waiting 是不限时的，也就是说无论过了多长时间它都不会主动恢复。
![blocked](./img/03/03_waitting_circulation.png)
想要从 Blocked 状态进入 Runnable 状态，要求线程获取 monitor 锁，而从 Waiting 状态流转到其他状态则比较特殊，因为首先 Waiting 是不限时的，也就是说无论过了多长时间它都不会主动恢复。
![blocked](./img/03/03_waitting_blocked_circulation.png)
...
![blocked](./img/03/03_timed_waitting_blocked_circulation.png)
同样在 Timed Waiting 中执行 notify() 和 notifyAll() 也是一样的道理，它们会先进入 Blocked 状态，然后抢夺锁成功后，再回到 Runnable 状态。
![blocked](./img/03/03_timed_waitting_circulation.png)

当然对于 Timed Waiting 而言，如果它的超时时间到了且能直接获取到锁/join的线程运行结束/被中断/调用了LockSupport.unpark()，会直接恢复到 Runnable 状态，而无需经历 Blocked 状态。

### Terminated 终止
![blocked](./img/03/03_Terminated_thread.png)
